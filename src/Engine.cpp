...
struct MsgPort* CreatePort(unsigned char* a, LONG b);
void DeletePort(MsgPort* port);
}
...
Bitmap* bm_create(WORD w, WORD h, WORD d, UBYTE* data)
{
    ...
    bm->rasterOwn = TRUE;
    bm->bitmap = (BitMap*)AllocMem(sizeof(struct BitMap), MEMF_CLEAR);
...
int main(void)
{
    ...
    old_processwinptr = (Window*)thisprocess->pr_WindowPtr;
    ...
    unsigned char id[] = "inputgamehandler";
    /*if ((InputMP = CreatePort(&id[0], 127L)))
    {
        if ((InputReq = (IOStdReq*)AllocMem(sizeof(struct IORequest), MEMF_PUBLIC | MEMF_CLEAR)))
        {
            ...
            if (OpenDevice("input.device", 0, (struct IORequest*)InputReq, 0) == 0)
            {
                ...
                InputHandler.is_Code = (void (*)())NullInputHandler;
                InputHandler.is_Node.ln_Name = (char*)&NameString[0];
    ...
    while((*(UBYTE*)0xBFE001) & 0x40)
    {
        ...
        sprintf((char*)numstr, "%d fps", fps_val);

        SetAPen(rastPort, 31);
        Move(rastPort, 10, 10);
        Text(rastPort, (char*)&numstr[0], strlen((char*)&numstr[0]));
    ...
    if (InputReq)
    {
        FreeMem(InputReq, sizeof(struct IORequest));
    }
    if (InputMP)
    {
        DeletePort(InputMP);
